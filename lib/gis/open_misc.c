...
static int G__open_misc(const char *dir, const char *element, const char *name,
                        const char *mapset, int mode)
{
    int fd;
    char path[GPATH_MAX];
    ...
    if (mode == 0) {
        if (G_name_is_fully_qualified(name, xname, xmapset)) {
            if (*mapset && strcmp(xmapset, mapset) != 0) {
                G_warning(_("G__open_misc(read): mapset <%s> doesn't match "
                            "xmapset <%s>"),
                          mapset, xmapset);
                return -1;
            }
        ...
        if ((fd = open(path, 0)) < 0)
            G_warning("G__open_misc(read): Unable to open '%s': %s", path,
                      strerror(errno));
        return fd;
    }
    /* WRITE */
    if (mode == 1 || mode == 2) {
        ...
        if (G_name_is_fully_qualified(name, xname, xmapset)) {
            if (strcmp(xmapset, mapset) != 0) {
                G_warning(
                    _("G__open_misc(write): xmapset <%s> != G_mapset() <%s>"),
                    xmapset, mapset);
                return -1;
            }
            name = xname;
